...
function createDeck() {
    let lastCardID = 0;
    return cards_1.default.map((card) => {
        return Object.assign(Object.assign({}, card), { id: lastCardID++ });
    });
}
let lastServerEventID = 0;
function createServerEvent(eventType, payload = {}) {
    return new database_1.GameEvent(lastServerEventID++, eventType, payload);
}
describe('GameState', () => {
    let deck = createDeck();
    let lastEventID = 0;
    const playerID = 0;
    const opponentID = 1;
    const createTestEvent = (type, payload = {}) => {
        return {
    ...
    describe('getServerState', () => {
        ...
        it('shuffles deck based on seed', () => {
            ...
            const state = game_1.GameState.fromEvents(events);
            assert_1.default.deepEqual(state.deck[0], {
                emissions: 500,
                id: 50,
                name: "vegan"
            });
            assert_1.default.deepEqual(state.deck[7], {
                emissions: 110,
                id: 8,
                name: "dator-paslagen"
            });
            assert_1.default.deepEqual(state.deck[10], {
                emissions: 500,
                id: 11,
                name: "duscha-15min"
            });
        ...
        it('sets player1 on first player connected', () => {
            ...
            const state = game_1.GameState.fromEvents(events);
            assert_1.default.deepEqual(state.player1, {
                socketID: playerID,
                hand: []
            });
        });
        describe('second player connected (game started)', () => {
            ...
            const beforeState = game_1.GameState.fromEvents(events);
            ...
            it('sets player2 on second player connect', () => {
                ...
                assert_1.default.deepEqual(state.player2.socketID, opponentID);
            });
            it('assigns player hands on second player connect', () => {
                ...
                if (!state.player1)
                    throw new Error("Player 1 undefined");
                assert_1.default.deepEqual(state.player1.hand, expectedP1Cards);
                if (!state.player2)
                    throw new Error("Player 2 undefined");
                assert_1.default.deepEqual(state.player2.hand, expectedP2Cards);
            });
            it('plays deck card to emissions line', () => {
                Array.of(1, 2, 3, 4, 5, 6).forEach(() => deck.pop());
                const card = deck.pop();
                assert_1.default.deepEqual(state.deck, deck);
                assert_1.default.deepEqual(state.emissionsLine, [card]);
            });
            it("sets player turn", () => {
                assert_1.default.equal(state.playerTurn, playerID);
            });
            it("notifies client of game start", () => {
                lastEventID = beforeState.clientEvents.length;
                assert_1.default.deepEqual(clientEvents.slice(1, clientEvents.length), [
        ...
        it('ignores third player', () => {
            ...
            const state2 = game_1.GameState.fromEvents(events);
            assert_1.default.deepEqual(state2, state);
        });
        it("sets state to waiting_for_players on first player connected", () => {
            const events = [
                createServerEvent("player_connected", { socketID: playerID })
            ];
            const clientEvents = game_1.GameState.fromEvents(events).clientEvents;
            assert_1.default.deepEqual(clientEvents, [
        ...
        it("ends game on disconnect", () => {
            ...
            const clientEvents = game_1.GameState.fromEvents(events).clientEvents;
            lastEventID = 10;
            assert_1.default.deepEqual(clientEvents[clientEvents.length - 1], createTestEvent("opponent_disconnected"));
        });
        describe('card played from hand', () => {
            let events;
            let player1;
            let player2;
            let emissionsLineCard;
            const playCardEvent = (cardID, position, socketID) => {
            ...
            describe("correct player's turn", () => {
                ...
                beforeEach(() => {
                    ...
                    state = game_1.GameState.fromEvents(events);
                });
                it("sets next player's turn", () => {
                    assert_1.default.equal(state.playerTurn, player2.socketID);
                });
                it("notifies client of turn change", () => {
                    lastEventID = 10;
                    assert_1.default.deepEqual(state.clientEvents[10], createTestEvent("player_turn", { socketID: player2.socketID }));
                });
            });
            describe("incorrect player's turn", () => {
                ...
                let state;
                it("ignores event", () => {
                    beforeState = game_1.GameState.fromEvents(events);
                    events.push(playCardEvent(player2.hand[0].id, 0, player2.socketID));
                    state = game_1.GameState.fromEvents(events);
                    assert_1.default.deepEqual(state, beforeState);
                });
            });
            describe("game over", () => {
                let state;
                ...
                it("notifies clients of winning player", () => {
                    ...
                    assert_1.default.deepEqual(state.clientEvents[state.clientEvents.length - 1], createTestEvent("game_won", { socketID: player1.socketID }));
                });
                it("does not allow cards to be played directly afterward", () => {
                    ...
                    state = game_1.GameState.fromEvents(events);
                    assert_1.default.deepEqual(state.emissionsLine, emissionsLineBefore);
                    const length = state.clientEvents.length;
                    lastEventID = length - 1;
                    assert_1.default.deepEqual(state.clientEvents[state.clientEvents.length - 1], createTestEvent("game_won", { socketID: player1.socketID }));
                });
                describe("vote new game", () => {
                    ...
                    it("notifies clients of player vote", () => {
                        const length = state.clientEvents.length;
                        lastEventID = length - 1;
                        assert_1.default.deepEqual(state.clientEvents[state.clientEvents.length - 1], createTestEvent("vote_new_game", { socketID: player1.socketID }));
                    });
                });
            });
            describe("placement correct", () => {
                ...
                it("moves card to emissions line", () => {
                    assert_1.default.deepEqual(state.emissionsLine, [card, card2, emissionsLineCard]);
                });
                it("moves card from hand", () => {
                    assert_1.default.deepEqual(player1.hand, player1.hand.filter(c => c != card));
                    assert_1.default.deepEqual(player2.hand, player2.hand.filter(c => c != card2));
                });
                it("notifies client", () => {
                    const clientEvents = state.clientEvents;
                    const length = clientEvents.length;
                    lastEventID = length - 3;
                    assert_1.default.deepEqual(clientEvents.slice(length - 3, length), [
            ...
            describe('placement incorrect', () => {
                ...
                it("does NOT move card to emissions line", () => {
                    assert_1.default.deepEqual(state.emissionsLine, [card, emissionsLineCard]);
                });
                it("moves card from player's hand and deal new card to hand", () => {
                    assert_1.default.deepEqual(player2.hand, [
                        ...player2Before.hand.filter((c) => c != card2),
                        newCard
                    ]);
                });
                it("moves card to bottom of deck", () => {
                    assert_1.default.deepEqual(state.deck[0], card2);
                });
                it("notifies clients of incorrect card placement", () => {
                    const clientEvents = state.clientEvents;
                    const length = clientEvents.length;
                    lastEventID = state.clientEvents.length - 4;
                    assert_1.default.deepEqual(clientEvents.slice(length - 4, length), [
                        createTestEvent("card_played_from_hand", event.payload),
                        createTestEvent("player_turn", { socketID: playerID }),
